...
export function Sidebar({ collapsed, onToggle, onThemeClick }: { collapsed: boolean; onToggle: () => void; onThemeClick: () => void }) {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        'flex items-center border-b border-sidebar-border h-[37px] shrink-0',
        collapsed ? 'justify-center px-0' : 'px-3',
      )}>
        {!collapsed && (
          <span className="text-[11px] font-medium text-accent tracking-wider uppercase cursor-pointer select-none" 
        ...
        {navItems.map(item => {
          ...
          return (
            <button type="button"
              ...
              className={cn(
                'flex items-center gap-2.5 w-full text-xs transition-colors relative group',
            ...
            >
              ...
              {!collapsed && <span className="truncate text-[11px] tracking-wide">{item.label}</span>}
              {active && (
                <span className={cn(
                  'absolute rounded-full bg-accent',
                  collapsed ? 'left-0.5 top-1/2 -translate-y-1/2 w-0.5 h-4' : 'left-0 top-1/2 -translate-y-1/2 w-0.5 h-4',
                )} />
              )}
              {collapsed && (
                <div className="absolute left-full ml-2 px-2 py-1 rounded-[4px] bg-base-900 border border-base-800 text-[11px] text-base-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50 pointer-events-none">
                  {item.label}
                </div>
              )}
            </button>
      ...
      <div className="border-t border-sidebar-border py-1">
        <button type="button"
          ...
          className={cn(
            'flex items-center gap-2.5 w-full text-xs transition-colors group',
            collapsed ? 'justify-center h-9' : 'px-3 h-8',
            'text-base-400 hover:text-base-200 hover:bg-base-800/50',
          )}
        ...
        >
          ...
          {!collapsed && <span className="truncate text-[11px] tracking-wide">Theme</span>}
          {collapsed && (
            <div className="absolute left-full ml-2 px-2 py-1 rounded-[4px] bg-base-900 border border-base-800 text-[11px] text-base-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50 pointer-events-none">
              Theme
            </div>
